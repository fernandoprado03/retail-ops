...
export default async function Pendientes() {
  ...
  return (
    <div className="pb-8">
      ...
      <div className="space-y-4 md:space-y-0 md:bg-white md:rounded-xl md:border md:border-slate-200 md:shadow-sm md:overflow-hidden">
        {incidencias?.length === 0 ? (
          <div className="text-center py-12 bg-white rounded-xl border border-slate-100 shadow-sm md:border-none">
          ...
          <>
            ...
            <div className="hidden md:flex items-center text-xs font-semibold text-slate-500 border-b border-slate-200 px-4 py-3 bg-slate-50/50">
              <div className="flex-1 pl-12">Detalle del Ticket</div>
              <div className="w-32">Reportado</div>
              <div className="w-40">Área Asignada</div>
              <div className="w-28">Estado</div>
            </div>

            <div className="flex flex-col md:divide-y md:divide-slate-100 space-y-4 md:space-y-0">
              {incidencias?.map((incidencia) => (
                <Link href={`/ticket/${incidencia.id}`} key={incidencia.id} className="block group">
                  <Card className="border-slate-100 shadow-sm hover:shadow-md transition-shadow bg-white overflow-hidden active:scale-[0.98] active:bg-slate-50 md:active:scale-100 md:border-none md:shadow-none md:rounded-none md:hover:bg-blue-50/50">
                    
                    {/* Mobile Layout */}
                    <div className="flex h-28 md:hidden">
                    ...
                    <div className="hidden md:flex items-center px-4 py-3 gap-4">
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        <div className="w-8 h-8 rounded-full bg-slate-100 shrink-0 flex items-center justify-center overflow-hidden border border-slate-200">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={incidencia.foto_antes_url} alt="Evidencia" className="w-full h-full object-cover" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-sm text-blue-600 group-hover:text-blue-800 transition-colors truncate">
                            {incidencia.tipo_incidencia}
                          </h3>
                          <p className="text-xs text-slate-500 truncate mt-0.5">{incidencia.descripcion}</p>
                        </div>
                      </div>
                      
                      <div className="w-32 text-xs text-slate-500 shrink-0 whitespace-nowrap">
                        {formatTime(incidencia.fecha_reporte)}
                      </div>
                      <div className="w-40 text-xs text-slate-700 font-medium shrink-0 truncate">
                        {incidencia.areas?.nombre_area || 'Área general'}
                      </div>
